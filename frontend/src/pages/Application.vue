<template>
    ...
    <div v-else-if="deals.data" class="flex h-full items-center justify-center">
      ...
      >
        ...
        <span>{{ __('No Applications Found') }}</span>
        <Button :label="__('Create')" @click="showApplicationModal = true">
          <template #prefix><FeatherIcon name="plus" class="h-4" /></template>
        </Button>
      </div>
    </div>
    <ApplicationModal v-model="showApplicationModal" />
  </template>
  ...
  const breadcrumbs = [{ label: __('Application'), route: { name: 'Application' } }]
