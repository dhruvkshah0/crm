<template>
    ...
    <div v-else-if="deals.data" class="flex h-full items-center justify-center">
      ...
      >
        ...
        <span>{{ __('No Merchants Found') }}</span>
        <Button :label="__('Create')" @click="showMerchantModal = true">
          <template #prefix><FeatherIcon name="plus" class="h-4" /></template>
        </Button>
      </div>
    </div>
    <MerchantModal v-model="showMerchantModal" />
  </template>
  ...
  const breadcrumbs = [{ label: __('Merchants'), route: { name: 'Merchants' } }]
