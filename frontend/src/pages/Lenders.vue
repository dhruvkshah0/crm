<template>
  <LayoutHeader>
    ...
    <template #right-header>
      ...
      />
      <Button variant="solid" :label="__('Add Lender')" @click="showLanderModal = true">
  ...
  <div v-else-if="deals.data" class="flex h-full items-center justify-center">
    <div class="flex flex-col items-center gap-3 text-xl font-medium text-gray-500">
      ...
      <span>{{ __("No Lenders Found") }}</span>
      <Button :label="__('Create')" @click="showLanderModal = true">
        <template #prefix><FeatherIcon name="plus" class="h-4" /></template>
      </Button>
    </div>
  </div>
  <LenderModal v-model="showLanderModal" />
</template>
...
const breadcrumbs = [{ label: __("Lenders"), route: { name: "Lenders" } }];
...
const showLanderModal = ref(false);
